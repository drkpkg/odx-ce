...
fn ensure_odoo_zip_downloaded(version: &str) {
    ...
    println!(
        "Odoo {} ZIP not found or invalid, attempting to download...",
        version
    );
            ...
            if output.status.success() && zip_path.exists() {
                ...
                if let Ok(metadata) = fs::metadata(&zip_path) {
                    if metadata.len() > 1_000_000 {
                        ...
                        println!(
                            "⚠  Downloaded file for {} is too small ({} bytes), may be invalid",
                            version,
                            metadata.len()
                        );
                    }
    ...
    println!(
        "⚠  Could not download valid ZIP for {}, will use git clone (slower)",
        version
    );
}
...
fn get_odoo_binary() -> PathBuf {
    ...
    if debug_path.exists() {
        ...
        if current_debug.exists() {
            ...
            panic!(
                "odx binary not found. Please build with 'cargo build' or 'cargo build --release'"
            );
        }
    }
}
...
fn test_doctor_command() {
    ...
    let output =
        output.unwrap_or_else(|_| panic!("Failed to execute doctor command. Binary: {bin}"));
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("System Requirements Check") || stdout.contains("Requirements"),
        "Should show system check. Output: {}",
        stdout
    );
}
...
fn test_doctor_json_mode_succeeds() {
    ...
    assert!(
        ...
        "Doctor should succeed in --json mode"
    );
}
...
fn test_new_project(version: &str, project_name: &str) {
    ...
    let bin = odoo_bin.display().to_string();
    let output = output.unwrap_or_else(|_| panic!("Failed to execute new command. Binary: {bin}"));
    ...
    assert!(
        ...
        "custom_addons should exist"
    );
    assert!(
        test_path.join("external_addons").exists(),
        "external_addons should exist"
    );
    assert!(
        test_path.join("compose.yml").exists(),
        "compose.yml should exist"
    );
    assert!(
        test_path.join("odoo.conf").exists(),
        "odoo.conf should exist"
    );
    assert!(
        test_path.join("README.md").exists(),
    ...
    if odoo_init.exists() {
        ...
        assert!(
            content.contains("version") || content.contains("__version__"),
    ...
    assert!(
        ...
        "Doctor should work in project"
    );

    let doctor_stdout = String::from_utf8_lossy(&doctor_output.stdout);
    assert!(
        doctor_stdout.contains("Odoo"),
        "Doctor should show Odoo information"
    );
}
...
fn test_doctor_in_project() {
    ...
    let bin = odoo_bin.display().to_string();
    let output = output.unwrap_or_else(|_| panic!("Failed to execute doctor. Binary: {bin}"));
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("Odoo") || stdout.contains("Requirements"),
        "Doctor should show Odoo or requirements information. Output: {}",
        stdout
    );
}
    ...
    for version in TEST_VERSIONS {
        ...
        if let Ok(output) = output {
            if output.status.success() {
                let stdout = String::from_utf8_lossy(&output.stdout);
                assert!(
                    stdout.contains("Odoo") || stdout.contains("version"),
                    "Doctor should show Odoo or version information for {}. Output: {}",
                    version,
                    stdout
                );
            }
    ...
    match output {
        Ok(output) => {
            ...
            assert!(
                ...
                "Install command should execute"
            );
        }
        Err(e) => {
            // If command not found, that's a real error
            panic!(
                "Install command failed to execute: {}. Binary: {}",
                e,
                odoo_bin.display()
            );
        }
...
fn test_commands_exist() {
    ...
    assert!(
        stdout.contains("new") || stdout.contains("New"),
        "Should have 'new' command. Output: {}",
        stdout
    );
    assert!(
        stdout.contains("doctor") || stdout.contains("Doctor"),
        "Should have 'doctor' command. Output: {}",
        stdout
    );
    assert!(
        stdout.contains("install") || stdout.contains("Install"),
        "Should have 'install' command. Output: {}",
        stdout
    );
    assert!(
        stdout.contains("run") || stdout.contains("Run"),
        "Should have 'run' command. Output: {}",
        stdout
    );
    assert!(
        stdout.contains("sync") || stdout.contains("Sync"),
        "Should have 'sync' command. Output: {}",
        stdout
    );
    ...
    assert!(
        i18n_help.status.success(),
        "'odx i18n --help' should succeed"
    );
